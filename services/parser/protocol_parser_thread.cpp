...
static parser::FrameAssembler g_frame;
static ModbusRtuReqAssembler g_hmi_rtu;
static parser::ProtocolRouter g_router;
    ...
    while (running_) {
        ...
        if (has_can) {
            ...
            if (r == proto::CanParseResult::Parsed) {
                ...
                // BMS async 正常路径：
                // CanDispatcher 已经把帧放入 BmsQueue，BmsWorker 会专线解析。
                // 这里不能报 ParseError，也不能再投递 DeviceData，避免重复链路。
            }
            else {
                ...
                LOG_THROTTLE_MS("can_parse_fail", 500, LOG_COMM_W,
                                ...
                                crx.can_index,
                                static_cast<unsigned>(crx.frame.can_id & CAN_EFF_MASK),
                                static_cast<int>(crx.frame.can_dlc),
                                static_cast<int>(r));
#endif
                if (on_parsed_) {
                    ParserMessage err;
        ...
        if (srx.type == dev::LinkType::RS485 || srx.type == dev::LinkType::RS232) {
            ...
            if (srx.type == dev::LinkType::RS485) {
                ...
                // ===== 从站口（HMI）：用 Modbus RTU 请求拼帧器处理 =====
                if (is_slave_port) {
                    g_hmi_rtu.onBytes(srx.serial_index, srx.bytes);

                    while (g_hmi_rtu.tryGetFrame(srx.serial_index, frame)) {
#if protocol_parser_thread_LOG
                        LOG_COMM_D("RX RS485#%d(SLAVE) len=%zu",
                                   srx.serial_index, frame.size());
                        LOG_COMM_HEX("RX", frame.data(), frame.size());
#endif

                        std::vector<uint8_t> tx;
                        const bool handled =
                            g_router.handleRs485Slave(srx.serial_index, frame, tx);

                        if (handled) {
                            static uint64_t last_hmi_heartbeat_ts = 0;
                            const uint64_t current_ts = nowMs();

                            // 每 1000ms 向控制层投递一次 HMI 活跃心跳
                            if (current_ts - last_hmi_heartbeat_ts > 1000) {
                                ...
                                if (on_parsed_) {
                                    ...
                                    heartbeat_msg.device_name = "HMI_SYS_HEARTBEAT";
...
#if protocol_parser_thread_LOG
                            LOG_COMM_D("TX RS485#%d(SLAVE) len=%zu",
                                       srx.serial_index, tx.size());
                ...
                while (g_ascii.tryGetFrame(srx.serial_index, frame)) {
                    ...
                    if (ok) {
                        ...
                        LOG_THROTTLE_MS("rs232_parse_fail_ups", 500, LOG_COMM_W,
                                        "RX RS232#%d dev=UPS parse=FAIL len=%zu",
                                        srx.serial_index, frame.size());
#endif
                        if (on_parsed_) {
                            ParserMessage err;
...
{
    if (!g_router.isRs485SlavePort(rs485_index)) {
        return nullptr;
    }
